...
fn decode_at(buf: &[u8], offset: usize) -> Result<(PackStreamValue, usize)> {
    ...
    match marker {
        ...
        0x90..=0x9F => {
            ...
            decode_list_body(buf, start, count)
        }

        // TinyDict: 0xA0–0xAF
        0xA0..=0xAF => {
            let count = usize::from(marker & 0x0F);
            decode_dict_body(buf, start, count)
        }
        ...
        0xB0..=0xBF => {
            ...
            let tag = buf[start];
            decode_struct_body(buf, start + 1, tag, field_count)
        }
        ...
        m::LIST8 => {
            ...
            decode_list_body(buf, start + 1, count)
        }

        m::LIST16 => {
            require_bytes_from(buf, start, 2)?;
            let count = usize::from(u16::from_be_bytes([buf[start], buf[start + 1]]));
            decode_list_body(buf, start + 2, count)
        }
        ...
        m::LIST32 => {
            ...
            decode_list_body(buf, start + 4, count)
        }

        m::DICT8 => {
            require_bytes_from(buf, start, 1)?;
            let count = usize::from(buf[start]);
            decode_dict_body(buf, start + 1, count)
        }

        m::DICT16 => {
            require_bytes_from(buf, start, 2)?;
            let count = usize::from(u16::from_be_bytes([buf[start], buf[start + 1]]));
            decode_dict_body(buf, start + 2, count)
        }
        ...
        m::DICT32 => {
            ...
            let raw =
                u32::from_be_bytes([buf[start], buf[start + 1], buf[start + 2], buf[start + 3]]);
            let count = raw as usize;
            decode_dict_body(buf, start + 4, count)
        }
...
fn decode_dict_body(
    ...
    for _ in 0..count {
        ...
        let (value, next2) = decode_at(buf, next)?;
...
fn decode_struct_body(
    ...
    for _ in 0..field_count {
        let (field, next) = decode_at(buf, offset)?;
        fields.push(field);
        offset = next;
    }
