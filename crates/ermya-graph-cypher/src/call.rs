...
use ermya_graph::Result;
use ermya_graph::gql::{CallStatement, Expr, Literal, ReturnClause, ReturnItem, UnwindClause};
...
/// # Errors
///
/// Returns [`ermya_graph::Error::GqlSyntaxError`] when the input begins with
...
mod tests {
    ...
    #[test]
    fn parse_ermya_vertex_labels_full() {
        let stmt = try_parse_call(
            "CALL ermya.vertex_labels() YIELD vertex_labels UNWIND vertex_labels AS vl RETURN vl",
        )
        .unwrap()
        .unwrap();
        assert_eq!(stmt.namespace.as_deref(), Some("ermya"));
    }
    ...
    #[test]
    fn parse_ermya_snapshot_two_string_args_no_yield() {
        use ermya_graph::gql::Literal;
        let stmt = try_parse_call("CALL ermya.snapshot('mydb', '/snapshots/mydb-1')")
            .unwrap()
            .unwrap();
        assert_eq!(stmt.namespace.as_deref(), Some("ermya"));
    ...
    #[test]
    fn parse_ermya_restore_double_quoted_args() {
        use ermya_graph::gql::Literal;
        let stmt = try_parse_call("CALL ermya.restore(\"mydb\", \"/snapshots/mydb-1\")")
            .unwrap()
            .unwrap();
