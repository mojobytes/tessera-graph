...
//! | `STARTS WITH`, `ENDS WITH`        | Passed     | Parsed unconditionally by the core GQL parser |
//! | `CONTAINS`                        | Passed     | Parsed unconditionally by the core GQL parser |
//! | `IN [list]`                       | Passed     | Parsed unconditionally by the core GQL parser |
//! | `id(n)`, `type(r)`, `labels(n)`   | Passed     | Parsed unconditionally by the core GQL parser |
...
//! | `UNWIND list AS var`              | Supported  | Parsed and compiled natively                  |

use ermya_graph::Error;
...
/// 6. Unsupported clauses (`WITH expr AS alias`) → informative error
///
...
pub fn cypher_to_gql(input: &str) -> ermya_graph::Result<String> {
...
pub fn reject_cypher_constructs(input: &str) -> ermya_graph::Result<()> {
...
fn strip_block_comments(input: &str) -> ermya_graph::Result<String> {
    let mut result = String::with_capacity(input.len());
    let chars: Vec<char> = input.chars().collect();
    let mut i = 0;
    while i < chars.len() {
...
fn convert_backtick_idents(input: &str) -> ermya_graph::Result<String> {
    let mut result = String::with_capacity(input.len());
    let chars: Vec<char> = input.chars().collect();
    let mut i = 0;
    while i < chars.len() {
...
fn rewrite_remove_clauses(input: &str) -> ermya_graph::Result<String> {
    let masked = blank_string_literals(input);
    let upper = masked.to_ascii_uppercase();
...
fn detect_unsupported_clauses(input: &str) -> ermya_graph::Result<()> {
    let masked = blank_string_literals(input);
    let upper = masked.to_ascii_uppercase();
...
mod tests {
    ...
    fn remove_clause_parses_as_set_mutation_in_cypher_compat_mode() {
        ...
        assert!(
            matches!(stmt, ermya_graph::GqlStatement::Mutation(_)),
    ...
    fn optional_match_in_cypher_compat_parses_as_query() {
        ...
        assert!(matches!(stmt, ermya_graph::GqlStatement::Query(_)));
    }
    ...
    #[test]
    fn unwind_in_cypher_compat_is_accepted() {
        let input = "UNWIND [1, 2, 3] AS x RETURN x";
        // UNWIND is now supported — preprocessor must pass it through.
        assert!(cypher_to_gql(input).is_ok(), "UNWIND should be accepted");
    }
