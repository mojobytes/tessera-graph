...
mod tests {
    ...
    fn cache_hit_returns_cloned_statement() {
        ...
        let stmt = ermya_graph::gql::parse_statement(q).unwrap(); // OK: test
    ...
    fn eviction_occurs_when_over_capacity() {
        ...
        let stmt = ermya_graph::gql::parse_statement("MATCH (n) RETURN n").unwrap(); // OK: test
        for i in 0..20 {
    ...
    fn cache_same_query_same_params_is_hit() {
        let cache = QueryCache::new(16);
        let q = "RETURN $x";
        let stmt = ermya_graph::gql::parse_statement("RETURN 1").unwrap(); // OK: test
