...
///
/// **Quote-aware**: newlines inside single-quoted string literals are preserved
/// verbatim and do not split statements.  This matches the GQL lexer's acceptance
/// of multiline strings and allows importing datasets produced by tools (e.g.
/// Memgraph) that emit newlines inside property values.
///
...
pub fn stream_gql_import<R: std::io::Read>(
    ...
    for line_result in reader.lines() {
        ...
        if in_string {
            ...
            in_string = !terminates_string(&line, in_string);
            if !in_string {
                // String closed on this line — check if the remainder
                // after the closing quote ends with `;` outside quotes.
                if ends_with_semicolon_outside_quotes(&current) {
                    let stmt = trim_trailing_semicolon(&current);
                    if !stmt.is_empty() {
                        on_stmt(stmt)?;
                        count += 1;
                    }
        ...
        // Check if this line leaves us inside an open string.
        in_string = has_open_string(trimmed);

        if !in_string && ends_with_semicolon_outside_quotes(&current) {
            let stmt = trim_trailing_semicolon(&current);
            if !stmt.is_empty() {
                on_stmt(stmt)?;
                count += 1;
...
/// Count unescaped single quotes in `s` and return whether the count is odd
/// (i.e. the line leaves us inside an open string literal).
///
/// Handles backslash escapes (`\'`) and doubled quotes (`''`).
fn has_open_string(s: &str) -> bool {
    let mut in_q = false;
    ...
    while i < len {
        ...
        if bytes[i] == b'\'' {
            if in_q && i + 1 < len && bytes[i + 1] == b'\'' {
                i += 2; // doubled quote — stays in string
                continue;
            }
            in_q = !in_q;
        }
...
/// Given that we are currently inside an open string, check whether
/// `line` closes it (i.e. contains the matching closing quote).
fn terminates_string(line: &str, was_in_string: bool) -> bool {
    if !was_in_string {
        return false;
    }
    // We enter already inside a string, so start in_q = true.
    let mut in_q = true;
    ...
    // If in_q is false, the string was closed on this line.
    !in_q
}

/// Check if `s` ends with `;` that is outside any string literal.
fn ends_with_semicolon_outside_quotes(s: &str) -> bool {
    let trimmed = s.trim_end();
    if !trimmed.ends_with(';') {
        return false;
    }
    // Verify the `;` is not inside a string by scanning from the start.
    !has_open_string(trimmed)
}

/// Strip the trailing `;` (and any whitespace before it) from a statement.
fn trim_trailing_semicolon(s: &str) -> String {
    let trimmed = s.trim_end();
    trimmed
        .strip_suffix(';')
        .unwrap_or(trimmed)
...
/// Generate GQL statements from a JSON string in ermya-import format.
///
/// Expected format:
/// ```json
/// {
...
mod tests {
    ...
    // --- Multiline string support in GQL splitter ---

    #[test]
    fn split_multiline_string_in_property() {
        // A property value that spans two lines — the newline is inside quotes.
        let input = "CREATE (:X {val: 'line1\nline2'});";
        let stmts = split_gql_statements(input);
        assert_eq!(stmts.len(), 1, "multiline string must not split statement");
        assert!(stmts[0].contains("line1\nline2"));
    }

    #[test]
    fn split_multiline_string_multiple_newlines() {
        let input = "CREATE (:X {val: 'a\nb\nc'});";
        let stmts = split_gql_statements(input);
        assert_eq!(stmts.len(), 1);
    ...
    #[test]
    fn split_multiline_string_followed_by_normal_statement() {
        let input = "CREATE (:X {val: 'multi\nline'});\nCREATE (:Y);";
        let stmts = split_gql_statements(input);
        assert_eq!(stmts.len(), 2);
    ...
    #[test]
    fn split_semicolon_inside_string_not_a_delimiter() {
        let input = "CREATE (:X {val: 'has;semicolon'});";
        let stmts = split_gql_statements(input);
        assert_eq!(stmts.len(), 1);
    ...
    #[test]
    fn split_multiline_string_with_blank_line_inside() {
        // Blank line inside string must be preserved, not skipped.
        let input = "CREATE (:X {val: 'before\n\nafter'});";
        let stmts = split_gql_statements(input);
        assert_eq!(stmts.len(), 1);
        assert!(stmts[0].contains("before\n\nafter"));
    }

    #[test]
    fn split_escaped_quote_does_not_close_string() {
        let input = "CREATE (:X {val: 'it\\'s fine'});";
        let stmts = split_gql_statements(input);
        assert_eq!(stmts.len(), 1);
    }

    #[test]
    fn split_doubled_quote_does_not_close_string() {
        let input = "CREATE (:X {val: 'it''s fine'});";
        let stmts = split_gql_statements(input);
        assert_eq!(stmts.len(), 1);
    }
    ...
    #[test]
    fn has_open_string_basic() {
        assert!(!has_open_string("no quotes here"));
        assert!(has_open_string("open 'string"));
        assert!(!has_open_string("closed 'string'"));
        assert!(has_open_string("'open"));
        assert!(!has_open_string("'closed'"));
    }

    #[test]
    fn has_open_string_escaped_quote() {
        assert!(has_open_string("'escaped\\'"));
        assert!(!has_open_string("'escaped\\' outside'"));
    }

    #[test]
    fn has_open_string_doubled_quote() {
        // '' inside string is an escape — string stays open after it
        assert!(has_open_string("'doubled''still open"));
        assert!(!has_open_string("'doubled''still open'"));
    }

    #[test]
    fn terminates_string_closes_on_line() {
        assert!(terminates_string("rest of value')", true));
        assert!(!terminates_string("still no close", true));
    }

    #[test]
    fn terminates_string_not_in_string() {
        assert!(!terminates_string("anything", false));
    }

    #[test]
    fn real_world_multiline_serial_number() {
        // Reproduces the actual dataset failure: a serial number with a newline.
        let input = concat!(
            "CREATE (:Component {`Serial_Number`: 'RMU/13/0269,\n",
            "RMU/13/0270', `name`: 'Switch'});\n",
            "CREATE (:Other {`id`: '123'});"
        );
        let stmts = split_gql_statements(input);
        assert_eq!(stmts.len(), 2, "expected 2 statements, got {stmts:?}");
        assert!(stmts[0].contains("RMU/13/0269,\nRMU/13/0270"));
        assert!(stmts[1].contains(":Other"));
    }
}
