...
use ermya_graph_cli_lib::admin;
use ermya_graph_cli_lib::auth;
use ermya_graph_cli_lib::cli::{AdminAction, Cli, Command};
use ermya_graph_cli_lib::config::ConnectionConfig;
use ermya_graph_cli_lib::connection::Session;
use ermya_graph_cli_lib::error::CliError;
use ermya_graph_cli_lib::export;
use ermya_graph_cli_lib::import::{self, ImportPlan};
use ermya_graph_cli_lib::output::OutputFormat;
use ermya_graph_cli_lib::query;
use ermya_graph_cli_lib::repl::{self, MetaCommand, QueryAccumulator};

use ermya_graph_protocol::BoltClient;
...
async fn run() -> Result<(), CliError> {
    ...
    if matches!(cli.command, Some(Command::Version)) {
        println!("ermya-cli {}", env!("CARGO_PKG_VERSION"));
        return Ok(());
    }
    ...
    W: AsyncWrite + Unpin,
{
    match command {
        Some(Command::Query(args)) => {
            ...
            let rendered = ermya_graph_cli_lib::output::render(
                format,
                &output.columns,
                &output.rows,
                Some(elapsed),
...
async fn handle_exec<R, W>(
    session: &mut Session<R, W>,
    args: &ermya_graph_cli_lib::cli::ExecArgs,
) -> Result<(), CliError>
where
    R: AsyncRead + Unpin,
    W: AsyncWrite + Unpin,
...
async fn handle_import<R, W>(
    session: &mut Session<R, W>,
    args: &ermya_graph_cli_lib::cli::ImportArgs,
) -> Result<(), CliError>
where
    R: AsyncRead + Unpin,
    W: AsyncWrite + Unpin,
...
async fn run_pipelined_import<R, W>(
    client: &mut ermya_graph_protocol::BoltClient<R, W>,
...
async fn handle_export<R, W>(
    session: &mut Session<R, W>,
    args: &ermya_graph_cli_lib::cli::ExportArgs,
) -> Result<(), CliError>
where
    R: AsyncRead + Unpin,
    W: AsyncWrite + Unpin,
        ...
        if let Some(completed_query) = accumulator.push(trimmed) {
            ...
            match query::execute_query(session, &completed_query, &language).await {
                Ok(output) => {
                    ...
                    match ermya_graph_cli_lib::output::render(
                        format,
                        &output.columns,
                        &output.rows,
                        elapsed,
...
/// Path to the history file: `~/.ermya_history`.
fn dirs_history_path() -> Option<std::path::PathBuf> {
    ermya_graph_cli_lib::config::home_dir().map(|h| h.join(".ermya_history"))
}
