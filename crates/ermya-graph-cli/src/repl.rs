...
///
/// When `continuation` is false, produces: `ermya[user@host:port]> `
...
pub fn format_prompt(username: &str, host: &str, port: u16, continuation: bool) -> String {
    let primary = format!("ermya[{username}@{host}:{port}]> ");
    if continuation {
...
mod tests {
    ...
    fn primary_prompt_format() {
        ...
        assert_eq!(p, "ermya[admin@localhost:7687]> ");
    }
    ...
    fn prompt_with_different_user() {
        ...
        assert_eq!(p, "ermya[bob@db.prod:9000]> ");
    }
