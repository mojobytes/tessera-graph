...
//! Integration tests for the `ermya-graph-cli admin …` offline commands.
//!
...
fn cli() -> Command {
    Command::new(env!("CARGO_BIN_EXE_ermya-graph-cli"))
}
...
fn cli_admin_help_lists_audit_log_flag() {
    ...
    assert!(
        stdout.contains("ERMYA_AUDIT_LOG"),
        "admin --help must mention ERMYA_AUDIT_LOG env, got: {stdout}"
    );
}
