...
fn test_context() -> ServerContext {
    ...
    let policy = Arc::new(AuthPolicy::new(user_store, RoleStoreHandle::with_defaults()));
...
fn permission_check_propagates_through_context() {
    ...
    let sessions = Arc::new(SessionManager::new(3600));
    let policy = Arc::new(AuthPolicy::new(user_store.clone(), RoleStoreHandle::with_defaults()));
    ...
    assert!(ctx2.check_permission(&token, Permission::NodeCreate).is_ok());

    // Original context's sessions don't have this token
    assert!(ctx.check_permission(&token, Permission::NodeCreate).is_err());
}
...
fn unauthenticated_request_is_denied() {
    ...
    assert!(ctx.check_permission(&fake_token, Permission::NodeRead).is_err());
}
