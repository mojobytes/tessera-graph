...
async fn serve_plain_handles_bolt_hello() {
    ...
    handshake[..4].copy_from_slice(&tessera_graph_protocol::BOLT_MAGIC);
    ...
    let mut cw = tessera_graph_protocol::bolt_frame::BoltChunkedWriter::new(write);
    let mut cr = tessera_graph_protocol::bolt_frame::BoltChunkedReader::new(read);
    ...
    let hello = BoltRequest::Hello {
        extra: vec![
            ...
            (
                ...
                tessera_graph_protocol::PackStreamValue::String("ignored".to_owned()),
            ),
            (
                "database".to_owned(),
                tessera_graph_protocol::PackStreamValue::String("tenanta".to_owned()),
            ),
        ],
    };
    ...
    let data = tessera_graph_protocol::encode_request(&goodbye).unwrap();
    cw.write_message(&data).await.unwrap();
...
async fn serve_plain_create_and_query_through_tcp() {
    ...
    // Bolt handshake.
    let mut hs = [0u8; 20];
    hs[..4].copy_from_slice(&tessera_graph_protocol::BOLT_MAGIC);
...
#[cfg(feature = "plain-tcp")]
async fn tcp_send(
    writer: &mut tessera_graph_protocol::bolt_frame::BoltChunkedWriter<
        tokio::io::WriteHalf<tokio::net::TcpStream>,
    >,
    req: &BoltRequest,
) {
    let data = tessera_graph_protocol::encode_request(req).unwrap();
    writer.write_message(&data).await.unwrap();
}

#[cfg(feature = "plain-tcp")]
async fn tcp_recv(
    reader: &mut tessera_graph_protocol::bolt_frame::BoltChunkedReader<
    ...
    tessera_graph_protocol::decode_response(&data).unwrap()
}
...
async fn open_bolt_session(
    ...
    tessera_graph_protocol::bolt_frame::BoltChunkedWriter<
        tokio::io::WriteHalf<tokio::net::TcpStream>,
    >,
    tessera_graph_protocol::bolt_frame::BoltChunkedReader<
        tokio::io::ReadHalf<tokio::net::TcpStream>,
    >,
) {
    let stream = tokio::net::TcpStream::connect(addr).await.unwrap();
    ...
    let hello = BoltRequest::Hello {
        extra: vec![
            ...
            (
                "database".to_owned(),
                tessera_graph_protocol::PackStreamValue::String(db.to_owned()),
            ),
        ],
    };
    let data = tessera_graph_protocol::encode_request(&hello).unwrap();
...
async fn serve_plain_caps_connections_per_ip() {
    ...
    let audit = tessera_graph_server::audit::AuditSink::file(
        audit_path.clone(),
        1_000_000,
        3,
        0,
    ...
    let rate_limiter = tessera_graph_server::rate_limiter::RateLimiter::new(64, 0, 2);
    // Montaje público: ni gestor de pago ni despachador de pago. El oyente
    // recibe el gestor por su interfaz y no distingue cuál le dan.
    tokio::spawn(async move {
        let _ = listener
