...
/// handler sent `TesseraGraph/Community`, which tripped this check and
...
async fn fresh_system_provider_with_user(
    username: &str,
    password: &str,
) -> std::sync::Arc<tessera_graph_server::auth::SystemGraphAuthProvider> {
    use tessera_graph_server::auth::{
        SecretString, SystemGraphAuthProvider, SystemGraphAuthStore, UserStore,
    };
    let graph = std::sync::Arc::new(std::sync::RwLock::new(tessera_graph::Graph::new()));
