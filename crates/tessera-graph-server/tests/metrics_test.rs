...
use tessera_graph_server::{ServerConfig, ServerHandle};
...
async fn metrics_endpoint_returns_200_with_prometheus_content_type() {
    ...
    let bound_addr = tessera_graph_server::metrics::spawn_metrics_server(addr, shutdown_rx)
        .await
...
async fn spawn_test_server() -> (
    ...
    SocketAddr,
    watch::Sender<bool>,
    tokio::task::JoinHandle<tessera_graph_server::Result<ServerHandle>>,
) {
...
/// Bolt 4.4 handshake.
///
/// The gauge `tessera_active_connections` is incremented in
...
async fn active_connections_gauge_increments_on_connect_and_decrements_on_close() {
    ...
    let before = snapshot(&metrics_addr, "tessera_active_connections", &[]).await;
    ...
    .await;
    let during = scrape_value(&metrics_addr, "tessera_active_connections", &[]).await;
    assert!(
        saw_increment,
...
fn label_guard_caps_database_label_to_other() {
    ...
    let guard = tessera_graph_server::metrics::LabelGuard::new();
    for i in 0..tessera_graph_server::metrics::METRICS_DATABASE_LABEL_CAP {
        ...
        assert_ne!(
            ...
            tessera_graph_server::metrics::METRICS_DATABASE_LABEL_CAP,
        );
    }
...
/// Drives the `tessera_wal_fsync_duration_seconds` histogram by running N
...
async fn spawn_test_server_with_slow_log(
    ...
    let cfg = ServerConfig {
        ...
        data_dir: Some(data_dir.clone()),
        password: Some("admin-pw-12chars".to_owned()),
        audit_sink: tessera_graph_server::config::AuditSinkKind::File,
...
// ── Task 4 Cycle 6: tessera_result_capped_total ─────────────────────────────
