...
async fn background_flush_task_flushes_dirty_graph() {
    ...
    let handle = spawn_background_flush(Arc::clone(&registry), 10, shutdown_rx, tessera_graph_monitor::AtomicHealthFlag::new());
...
async fn background_flush_zero_interval_returns_noop() {
    ...
    let handle = spawn_background_flush(Arc::clone(&registry), 0, shutdown_rx, tessera_graph_monitor::AtomicHealthFlag::new());
