    ...
    A: AuthProvider + 'static,
{
    ...
    let query_cache = Arc::new(tessera_graph_cypher::cache::QueryCache::new(256));
...
#[allow(dead_code)]
pub fn default_graph() -> Arc<tessera_graph_server::DefaultGraphAccessor> {
    use std::sync::RwLock;
    let graph = Arc::new(RwLock::new(Graph::new()));
    Arc::new(tessera_graph_server::DefaultGraphAccessor::new(graph))
}
...
pub async fn prepopulate_system(data_dir: &std::path::Path, admin_password: &str) {
    ...
    use tessera_graph_server::migration::{CURRENT_DISK_LAYOUT, write};
    ...
    write(data_dir, CURRENT_DISK_LAYOUT).expect("stamp tessera.version marker");
}
...
/// [`tessera_graph_server::DefaultGraphAccessor`] would after the engine aborts
...
impl GraphAccessor for TimeoutAccessor {
    fn execute_query(
        &self,
        _query: &tessera_graph::gql::GqlQuery,
        _params: std::collections::HashMap<String, tessera_graph::gql::GqlValue>,
        _max_rows: u64,
        _deadline: Option<std::time::Instant>,
    ) -> Result<Vec<tessera_graph_server::graph_accessor::ResultRow>, String> {
        ...
        Err(format!(
            ...
            tessera_graph_server::graph_accessor::ENGINE_QUERY_TIMEOUT_PREFIX,
    ...
    fn execute_mutation(
        &self,
        _mutation: &tessera_graph::gql::MutationStatement,
        _params: std::collections::HashMap<String, tessera_graph::gql::GqlValue>,
        _deadline: Option<std::time::Instant>,
    ) -> Result<
        (
            Vec<tessera_graph_server::graph_accessor::ResultRow>,
            tessera_graph::gql::GqlMutationResult,
        ),
        String,
    > {
        Ok((Vec::new(), tessera_graph::gql::GqlMutationResult::default()))
    }

    fn execute_pipeline(
        &self,
        _pq: &tessera_graph::gql::PipelineQuery,
        _params: std::collections::HashMap<String, tessera_graph::gql::GqlValue>,
        _max_rows: u64,
        _deadline: Option<std::time::Instant>,
    ) -> Result<
    ...
    fn execute_const_return(
        &self,
        _q: &tessera_graph::gql::ConstReturnQuery,
        _params: std::collections::HashMap<String, tessera_graph::gql::GqlValue>,
        _max_rows: u64,
        _deadline: Option<std::time::Instant>,
    ) -> Result<Vec<tessera_graph_server::graph_accessor::ResultRow>, String> {
...
pub async fn spawn_bolt_handler_no_auth_with_timeout_double(
    ...
    let factory: tessera_graph_server::AccessorFactory =
...
pub async fn start_community_server_with_ready(
    cfg: tessera_graph_server::ServerConfig,
    shutdown_rx: tokio::sync::watch::Receiver<bool>,
) -> (
    tokio::task::JoinHandle<tessera_graph_server::Result<tessera_graph_server::ServerHandle>>,
    tessera_graph_server::ServerReady,
) {
...
pub async fn start_community_server(
    cfg: tessera_graph_server::ServerConfig,
    shutdown_rx: tokio::sync::watch::Receiver<bool>,
) -> tessera_graph_server::Result<tessera_graph_server::ServerHandle> {
    tessera_graph_server::start_server_with_registry(
        cfg,
        shutdown_rx,
        None,
...
pub struct AuthRateLimitFixture {
    auth: Arc<tessera_graph_server::auth::SystemGraphAuthProvider>,
    auth_store: Arc<dyn UserStore>,
    audit: AuditSink,
    audit_shutdown_tx: tokio::sync::watch::Sender<bool>,
    registry: Arc<dyn GraphRegistry>,
    rate_limiter: Arc<tessera_graph_server::rate_limiter::RateLimiter>,
...
impl AuthRateLimitFixture {
    ...
    pub async fn new(auth_cap: u32) -> Self {
        ...
        use tessera_graph_server::auth::{SecretString, SystemGraphAuthProvider};
        use tessera_graph_server::rate_limiter::RateLimiter;
