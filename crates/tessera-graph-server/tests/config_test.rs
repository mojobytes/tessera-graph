...
use tessera_graph_server::config::{AuditSinkKind, ServerConfig};
...
fn server_config_default_has_expected_values() {
    ...
    assert_eq!(cfg.data_dir, Some(PathBuf::from("/var/lib/tessera/data")));
...
fn from_map_parses_server_agent() {
    ...
    m.insert("TESSERA_SERVER_AGENT".into(), "TesseraGraph/0.7.0".into());
    let cfg = ServerConfig::from_map(&m);
    assert_eq!(cfg.server_agent, "TesseraGraph/0.7.0");
}
...
fn from_map_parses_all_fields() {
    let mut m = HashMap::new();
    m.insert("TESSERA_BIND".into(), "0.0.0.0:9999".into());
    m.insert("TESSERA_TLS_CERT".into(), "/tmp/cert.pem".into());
    m.insert("TESSERA_TLS_KEY".into(), "/tmp/key.pem".into());
    m.insert("TESSERA_PASSWORD".into(), "s3cret".into());
    m.insert("TESSERA_DATA_DIR".into(), "/data/tessera".into());
    m.insert("TESSERA_MAX_CONNECTIONS".into(), "512".into());
    m.insert("TESSERA_IDLE_TIMEOUT".into(), "600".into());
    ...
    assert_eq!(cfg.data_dir, Some(PathBuf::from("/data/tessera")));
...
fn max_txn_memory_bytes_parsed_from_env() {
    let m = HashMap::from([(
        "TESSERA_MAX_TXN_MEMORY_BYTES".to_owned(),
        "1048576".to_owned(),
    )]);
    let cfg = ServerConfig::from_map(&m);
...
fn from_map_ignores_invalid_numeric_values_and_uses_defaults() {
    let mut m = HashMap::new();
    m.insert("TESSERA_MAX_CONNECTIONS".into(), "not-a-number".into());
    m.insert("TESSERA_IDLE_TIMEOUT".into(), String::new());
...
fn from_map_partial_override() {
    let mut m = HashMap::new();
    m.insert("TESSERA_BIND".into(), "0.0.0.0:7688".into());
    m.insert("TESSERA_PASSWORD".into(), "pw".into());
...
fn audit_sink_off_parses() {
    ...
    vars.insert("TESSERA_AUDIT_SINK".to_owned(), "off".to_owned());
...
fn audit_sink_file_parses() {
    let mut vars = HashMap::new();
    vars.insert("TESSERA_AUDIT_SINK".to_owned(), "file".to_owned());
    vars.insert(
        "TESSERA_AUDIT_FILE".to_owned(),
        "/var/log/tessera/audit.log".to_owned(),
    );
    vars.insert("TESSERA_AUDIT_MAX_BYTES".to_owned(), "50".to_owned());
    vars.insert("TESSERA_AUDIT_KEEP_FILES".to_owned(), "3".to_owned());
    vars.insert("TESSERA_AUDIT_FSYNC_EVERY".to_owned(), "100".to_owned());
    let cfg = ServerConfig::from_map(&vars);
    assert_eq!(cfg.audit_sink, AuditSinkKind::File);
    assert_eq!(
        cfg.audit_file,
        Some(PathBuf::from("/var/log/tessera/audit.log"))
    );
...
fn audit_sink_unknown_falls_back_to_stdout() {
    let mut vars = HashMap::new();
    vars.insert("TESSERA_AUDIT_SINK".to_owned(), "cloudwatch".to_owned());
    let cfg = ServerConfig::from_map(&vars);
    assert_eq!(cfg.audit_sink, AuditSinkKind::Stdout);
}
...
fn no_auth_flag_requires_exact_string_one() {
    let mut vars = HashMap::new();
    vars.insert("TESSERA_NO_AUTH".to_owned(), "1".to_owned());
    assert!(ServerConfig::from_map(&vars).no_auth);

    vars.insert("TESSERA_NO_AUTH".to_owned(), "true".to_owned());
    assert!(!ServerConfig::from_map(&vars).no_auth);

    vars.insert("TESSERA_NO_AUTH".to_owned(), "yes".to_owned());
    assert!(!ServerConfig::from_map(&vars).no_auth);

    vars.insert("TESSERA_NO_AUTH".to_owned(), "0".to_owned());
    assert!(!ServerConfig::from_map(&vars).no_auth);
}
...
fn metrics_addr_parsed_from_env_var() {
    let mut m = HashMap::new();
    m.insert("TESSERA_METRICS_ADDR".into(), "0.0.0.0:9090".into());
    let cfg = ServerConfig::from_map(&m);
...
fn max_result_rows_parsed_from_env_map() {
    let map = HashMap::from([("TESSERA_MAX_RESULT_ROWS".to_owned(), "250000".to_owned())]);
    let cfg = ServerConfig::from_map(&map);
...
fn max_result_rows_zero_disables_cap() {
    let map = HashMap::from([("TESSERA_MAX_RESULT_ROWS".to_owned(), "0".to_owned())]);
    let cfg = ServerConfig::from_map(&map);
    assert_eq!(
...
fn max_result_rows_invalid_env_falls_back_to_default() {
    let map = HashMap::from([(
        "TESSERA_MAX_RESULT_ROWS".to_owned(),
        "not-a-number".to_owned(),
    )]);
    let cfg = ServerConfig::from_map(&map);
...
fn queries_max_per_second_parsed_from_env_map() {
    let mut vars = HashMap::new();
    vars.insert("TESSERA_QUERIES_MAX_PER_SECOND".to_owned(), "50".to_owned());
    let cfg = ServerConfig::from_map(&vars);
...
fn max_connections_per_ip_parsed_from_env_map() {
    let mut vars = HashMap::new();
    vars.insert("TESSERA_MAX_CONNECTIONS_PER_IP".to_owned(), "8".to_owned());
    let cfg = ServerConfig::from_map(&vars);
...
fn rate_limit_ip_cap_parsed_from_env_map() {
    let mut vars = HashMap::new();
    vars.insert("TESSERA_RATE_LIMIT_IP_CAP".to_owned(), "512".to_owned());
    let cfg = ServerConfig::from_map(&vars);
...
fn query_timeout_ms_parsed_from_env_map() {
    let map = HashMap::from([("TESSERA_QUERY_TIMEOUT_MS".to_owned(), "5000".to_owned())]);
    let cfg = ServerConfig::from_map(&map);
...
fn query_timeout_ms_zero_keeps_timeout_disabled() {
    let map = HashMap::from([("TESSERA_QUERY_TIMEOUT_MS".to_owned(), "0".to_owned())]);
    let cfg = ServerConfig::from_map(&map);
    assert_eq!(
        cfg.query_timeout_ms, 0,
...
fn query_timeout_ms_invalid_env_falls_back_to_default() {
    let map = HashMap::from([(
        "TESSERA_QUERY_TIMEOUT_MS".to_owned(),
        "not-a-number".to_owned(),
    )]);
    let cfg = ServerConfig::from_map(&map);
    assert_eq!(
...
fn query_timeout_ms_does_not_perturb_other_defaults() {
    let map = HashMap::from([("TESSERA_QUERY_TIMEOUT_MS".to_owned(), "250".to_owned())]);
    let cfg = ServerConfig::from_map(&map);
...
fn file_config_is_ignored_when_no_toml_exists() {
    ...
    let cfg = ServerConfig::from_file_and_env("/nonexistent/tessera.toml", &HashMap::new());
    assert_eq!(
        cfg.bind_addr,
...
fn file_config_env_overrides_toml() {
    ...
    let env = HashMap::from([("TESSERA_MAX_CONNECTIONS".to_owned(), "128".to_owned())]);
...
fn data_dir_default_is_file_backed() {
    ...
    assert_eq!(
        cfg.data_dir,
        Some(PathBuf::from("/var/lib/tessera/data")),
...
fn data_dir_memory_sentinel_produces_none() {
    let m = HashMap::from([("TESSERA_DATA_DIR".to_owned(), ":memory:".to_owned())]);
    let cfg = ServerConfig::from_map(&m);
    assert!(
        cfg.data_dir.is_none(),
        "TESSERA_DATA_DIR=:memory: must select in-memory mode (None)"
    );
}

#[test]
fn data_dir_explicit_path_overrides_default() {
    let m = HashMap::from([("TESSERA_DATA_DIR".to_owned(), "/custom/path".to_owned())]);
    let cfg = ServerConfig::from_map(&m);
...
fn data_dir_absent_key_uses_file_backed_default() {
    // Omitting TESSERA_DATA_DIR must NOT mean in-memory — it uses the default.
    let cfg = ServerConfig::from_map(&HashMap::new());
    assert_eq!(
        cfg.data_dir,
        Some(PathBuf::from("/var/lib/tessera/data")),
        "an unset TESSERA_DATA_DIR must fall back to the file-backed default"
    );
}
