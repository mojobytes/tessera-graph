...
//! `TesseraGraph` Server — standalone entry point.
...
use tessera_graph_server::edition_community::{parse_paid_settings, start_server};
...
/// `TESSERA_CONFIG_FILE` environment variable.
const DEFAULT_CONFIG_PATH: &str = "/etc/tessera/tessera.toml";
...
///
/// Resolution:
/// - If `TESSERA_CONFIG_FILE` is set, that path is authoritative: a missing
///   or unreadable file there is an operator error and aborts startup.
/// - Otherwise the default `/etc/tessera/tessera.toml` is consulted; its
///   absence is normal (env-only deployments) and is not an error.
fn load_config() -> ServerConfig {
    let env: HashMap<String, String> = std::env::vars()
        .filter(|(k, _)| k.starts_with("TESSERA_") && k != "TESSERA_CONFIG_FILE")
        .collect();

    match std::env::var("TESSERA_CONFIG_FILE") {
        Ok(path) => {
            if !std::path::Path::new(&path).is_file() {
                eprintln!(
                    "TESSERA_CONFIG_FILE points to '{path}' but no readable file \
                     exists there; refusing to start with a misconfigured path"
                );
                std::process::exit(1);
            }
            info!(config_file = %path, "loading configuration from TESSERA_CONFIG_FILE");
...
///
/// `TESSERA_LOG_FORMAT=json` switches to the `tracing-subscriber` JSON
...
/// from `TESSERA_LOG_FILTER` first, then the legacy `RUST_LOG`, then a
/// default of `info`. An unparseable filter falls back to `info` rather
/// than failing startup.
fn init_tracing() {
    let filter_str = std::env::var("TESSERA_LOG_FILTER")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "info".to_owned());
    let env_filter = tracing_subscriber::EnvFilter::try_new(&filter_str)
        .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new("info"));
    if std::env::var("TESSERA_LOG_FORMAT").as_deref() == Ok("json") {
