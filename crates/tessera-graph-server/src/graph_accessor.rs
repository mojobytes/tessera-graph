...
/// underlying [`tessera_graph::Error`] is a `QuotaExceeded`. The Bolt
...
/// `tessera_graph::Error` through the trait, which would touch every
...
/// [`tessera_graph::gql::RESULT_CAP_MSG_PREFIX`], or the output-row guard
...
/// [`tessera_graph::gql::TIMEOUT_MSG_PREFIX`]). The Bolt handler matches this
...
/// (`tessera_graph::Error::ConstraintViolation`). The Bolt handler matches
...
/// operation-count or byte cap (`tessera_graph::Error::BatchLimitExceeded`).
...
/// per-transaction memory cap (`tessera_graph::Error::TxnMemoryCapExceeded`).
...
/// (`tessera_graph::Error::DeleteConnectedNode`). The Bolt handler matches this
...
/// label declared append-only (`tessera_graph::Error::AppendOnlyLabelInTxn`,
...
/// Convert a `tessera_graph::Error` into the `String` form that the
...
pub(crate) fn engine_err_to_string(e: tessera_graph::Error) -> String {
    if matches!(e, tessera_graph::Error::QuotaExceeded { .. }) {
        format!("{ENGINE_QUOTA_EXCEEDED_PREFIX}{e}")
    } else if matches!(e, tessera_graph::Error::ConstraintViolation { .. }) {
        format!("{ENGINE_CONSTRAINT_VIOLATED_PREFIX}{e}")
    } else if matches!(e, tessera_graph::Error::AppendOnlyLabelInTxn { .. }) {
        format!("{ENGINE_APPEND_ONLY_IN_TXN_PREFIX}{e}")
    } else if matches!(e, tessera_graph::Error::DeleteConnectedNode { .. }) {
        format!("{ENGINE_DELETE_CONNECTED_PREFIX}{e}")
    } else if matches!(e, tessera_graph::Error::TxnMemoryCapExceeded { .. }) {
        format!("{ENGINE_TXN_MEMORY_CAP_PREFIX}{e}")
    } else if matches!(e, tessera_graph::Error::BatchLimitExceeded { .. }) {
        format!("{ENGINE_BATCH_LIMIT_PREFIX}{e}")
    } else if matches!(&e, tessera_graph::Error::GqlCompileError(m)
        if m.starts_with(tessera_graph::gql::RESULT_CAP_MSG_PREFIX))
    {
        format!("{ENGINE_RESULT_CAPPED_PREFIX}{e}")
    } else if matches!(&e, tessera_graph::Error::GqlCompileError(m)
        if m.starts_with(tessera_graph::gql::TIMEOUT_MSG_PREFIX))
    {
...
pub trait GraphAccessor: Send + Sync + 'static {
    ...
    /// [`tessera_graph::gql::param_substitution::apply`] to `query`, so the
...
impl GraphAccessor for DefaultGraphAccessor {
    ...
    fn execute_pipeline(
        ...
        use tessera_graph::gql::PipelineTerminal;
...
mod tests {
    ...
    use tessera_graph::gql::{self, GqlStatement, GqlValue};
    ...
    fn find_person(accessor: &DefaultGraphAccessor, name: &str) -> tessera_graph::NodeId {
    ...
    fn match_create_edge_with_properties() {
        ...
        assert_eq!(
            since,
            Some(tessera_graph::Property::I64(2024)),
    ...
    fn engine_err_to_string_remaps_result_cap_marker() {
        let e = tessera_graph::Error::GqlCompileError(format!(
            "{}query produced 99 rows",
            tessera_graph::gql::RESULT_CAP_MSG_PREFIX
        ));
    ...
    fn engine_err_to_string_remaps_delete_connected_node_marker() {
        let e = tessera_graph::Error::DeleteConnectedNode {
            node: tessera_graph::NodeId::from_raw(7),
            relationships: 3,
        };
        let s = super::engine_err_to_string(e);
        assert!(
    ...
    fn engine_err_to_string_remaps_txn_memory_cap_marker() {
        let e = tessera_graph::Error::TxnMemoryCapExceeded {
            txn_id: 4,
            used_bytes: 2048,
            cap_bytes: 1024,
        };
    ...
    fn engine_err_to_string_remaps_batch_limit_marker() {
        let e = tessera_graph::Error::BatchLimitExceeded {
            kind: tessera_graph::BatchLimitKind::Operations,
            current: 5000,
            limit: 1000,
        };
        let s = super::engine_err_to_string(e);
    ...
    fn engine_err_to_string_leaves_ordinary_compile_error() {
        let e = tessera_graph::Error::GqlCompileError("ordinary scope error".to_owned());
        let s = super::engine_err_to_string(e);
    ...
    fn engine_err_to_string_remaps_query_timeout_marker() {
        ...
        let e = tessera_graph::Error::GqlCompileError(format!(
            ...
            tessera_graph::gql::TIMEOUT_MSG_PREFIX
        ));
        let s = super::engine_err_to_string(e);
        assert!(
