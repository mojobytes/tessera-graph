...
//!
//! 1. `tessera-graph-server` startup — held for the lifetime of the
//!    running server so two server processes can never share identity
//!    state.
//! 2. `tessera-graph-cli admin users …` — held while the offline admin
//!    command opens the system graph; if the server is up, the lock is
//!    contended and the CLI exits with code 3.
//! 3. `tessera-graph-cli migrate` — added by Task 12 QR-5 so an
