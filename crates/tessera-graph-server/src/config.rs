...
/// Configuration for the `TesseraGraph` server.
///
...
pub struct ServerConfig {
    ...
    /// Bypass authentication (`TESSERA_NO_AUTH=1`). Emits a warn on startup.
    pub no_auth: bool,
    ...
    /// `tessera_open_databases` and the per-database
    /// `tessera_database_size_bytes` gauges, and recycles label-guard
    ...
    /// path). Default 1000 ms. Env: `TESSERA_SLOW_QUERY_THRESHOLD_MS`.
    ...
    /// events/minute. Env: `TESSERA_SLOW_QUERY_MAX_EVENTS_PER_MINUTE`.
    ...
    /// pipeline expansion). Env: `TESSERA_MAX_RESULT_ROWS`.
    ...
    /// Default `5`. Env: `TESSERA_AUTH_MAX_FAILURES_PER_MINUTE`.
    ...
    /// `0` disables. Default `100`. Env: `TESSERA_QUERIES_MAX_PER_SECOND`.
    ...
    /// `0` disables. Default `16`. Env: `TESSERA_MAX_CONNECTIONS_PER_IP`.
    ...
    /// timeout — opt-in. Env: `TESSERA_QUERY_TIMEOUT_MS`.
    ...
    /// without patching the driver. Override with `TESSERA_SERVER_AGENT` for
    /// custom branding; the Neo4j .NET driver only requires a valid semver
    /// after the slash. Env: `TESSERA_SERVER_AGENT`.
...
impl Default for ServerConfig {
    fn default() -> Self {
        Self {
            ...
            // explicit choice via TESSERA_DATA_DIR=:memory:). Unix production
            // path; override with TESSERA_DATA_DIR on other platforms.
            data_dir: Some(PathBuf::from("/var/lib/tessera/data")),
            max_connections: 256,
...
impl ServerConfig {
    ...
    ///
    /// Unknown values for `TESSERA_AUDIT_SINK` fall back to the default
    /// (`Stdout`) rather than failing; invalid numbers follow the same
    /// fallback-to-default pattern already used for `TESSERA_MAX_CONNECTIONS`.
    #[must_use]
    pub fn from_map(vars: &HashMap<String, String>) -> Self {
        ...
        Self {
            ...
            tls_cert: vars.get("TESSERA_TLS_CERT").map(PathBuf::from),
            tls_key: vars.get("TESSERA_TLS_KEY").map(PathBuf::from),
            password: vars.get("TESSERA_PASSWORD").cloned(),
            data_dir: parse_data_dir(vars.get("TESSERA_DATA_DIR"), defaults.data_dir),
            max_connections: parse_num(vars, "TESSERA_MAX_CONNECTIONS", defaults.max_connections),
            idle_timeout_secs: parse_num(vars, "TESSERA_IDLE_TIMEOUT", defaults.idle_timeout_secs),
            audit_sink: parse_audit_sink(vars.get("TESSERA_AUDIT_SINK"))
            ...
            no_auth: vars.get("TESSERA_NO_AUTH").map(String::as_str) == Some("1"),
            // Optional: absence (or unparsable value) means "unlimited".
            max_txn_memory_bytes: parse_opt_num(vars, "TESSERA_MAX_TXN_MEMORY_BYTES"),
            ...
            // limit".
            max_batch_operations: parse_opt_num(vars, "TESSERA_MAX_BATCH_OPERATIONS")
                .or(defaults.max_batch_operations),
            max_batch_memory_bytes: parse_opt_num(vars, "TESSERA_MAX_BATCH_MEMORY_BYTES")
            ...
            // string here.
            metrics_addr: vars.get("TESSERA_METRICS_ADDR").cloned(),
            ...
            max_result_rows: parse_num(vars, "TESSERA_MAX_RESULT_ROWS", defaults.max_result_rows),
            auth_max_failures_per_minute: parse_num(
                vars,
                "TESSERA_AUTH_MAX_FAILURES_PER_MINUTE",
                defaults.auth_max_failures_per_minute,
            ),
            queries_max_per_second: parse_num(
                vars,
                "TESSERA_QUERIES_MAX_PER_SECOND",
                defaults.queries_max_per_second,
            ),
            max_connections_per_ip: parse_num(
                vars,
                "TESSERA_MAX_CONNECTIONS_PER_IP",
    ...
    pub fn from_env() -> Self {
        let vars: HashMap<String, String> = std::env::vars()
            .filter(|(k, _)| k.starts_with("TESSERA_"))
            .collect();
        Self::from_map(&vars)
    }
...
/// Resolve `TESSERA_DATA_DIR` into the optional data directory.
///
...
/// Parse a single `TESSERA_*` value as `T`, falling back to `default` when the
...
/// to exactly one `TESSERA_*` key consumed by [`ServerConfig::from_map`], so
...
/// Translate a parsed [`FileConfig`] into the `TESSERA_*` key/value map that
...
fn file_config_to_map(fc: &FileConfig) -> HashMap<String, String> {
    ...
    put_str("TESSERA_BIND", &fc.bind);
    put_str("TESSERA_TLS_CERT", &fc.tls_cert);
    put_str("TESSERA_TLS_KEY", &fc.tls_key);
    put_str("TESSERA_PASSWORD", &fc.password);
    put_str("TESSERA_DATA_DIR", &fc.data_dir);
    put_str("TESSERA_AUDIT_SINK", &fc.audit_sink);
    put_str("TESSERA_AUDIT_FILE", &fc.audit_file);
    put_str("TESSERA_METRICS_ADDR", &fc.metrics_addr);
    put_str("TESSERA_SERVER_AGENT", &fc.server_agent);
    ...
    put_num("TESSERA_MAX_CONNECTIONS", fc.max_connections);
    put_num("TESSERA_IDLE_TIMEOUT", fc.idle_timeout);
    put_num("TESSERA_AUDIT_MAX_BYTES", fc.audit_max_bytes);
    ...
    put_num("TESSERA_MAX_RESULT_ROWS", fc.max_result_rows);
    put_num(
        "TESSERA_AUTH_MAX_FAILURES_PER_MINUTE",
        fc.auth_max_failures_per_minute,
    );
    put_num("TESSERA_QUERIES_MAX_PER_SECOND", fc.queries_max_per_second);
    put_num("TESSERA_MAX_CONNECTIONS_PER_IP", fc.max_connections_per_ip);
    put_num("TESSERA_MAX_BYTES_PER_SECOND", fc.max_bytes_per_second);
    put_num("TESSERA_RATE_LIMIT_IP_CAP", fc.rate_limit_ip_cap);
    put_num("TESSERA_QUERY_TIMEOUT_MS", fc.query_timeout_ms);

    // `no_auth` is a bool in TOML but `from_map` expects the literal "1".
    if fc.no_auth == Some(true) {
        m.insert("TESSERA_NO_AUTH".to_owned(), "1".to_owned());
    }
    m
}
