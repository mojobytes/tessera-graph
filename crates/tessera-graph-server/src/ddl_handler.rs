...
fn persist_error(e: &tessera_graph::Error) -> (String, String) {
    (
        "Neo.ClientError.Statement.ExecutionFailed".to_owned(),
...
mod tests {
    ...
    fn set_label_append_only_declares_in_catalog() {
        let g = make_graph();
        let stmt = tessera_graph::gql::DdlStatement::SetLabelAppendOnly {
    ...
    fn remove_label_append_only_withdraws_and_frees_existing_nodes() {
        let g = make_graph();
        dispatch_ddl(
            tessera_graph::gql::DdlStatement::SetLabelAppendOnly {
                label: "Event".to_owned(),
                on: true,
            },
            &g,
        ...
        let id = {
            ...
            guard
                .add_node("Event", tessera_graph::Properties::new())
                .unwrap()
        };
    ...
    fn show_append_only_info_lists_declared_labels() {
        ...
        let result =
            dispatch_ddl(tessera_graph::gql::DdlStatement::ShowAppendOnlyInfo, &g).unwrap();
    ...
    fn create_index_legacy_adds_to_catalog() {
        let g = make_graph();
        let stmt = tessera_graph::gql::DdlStatement::CreateIndexLegacy {
    ...
    fn create_index_for_adds_to_catalog() {
        let g = make_graph();
        let stmt = tessera_graph::gql::DdlStatement::CreateIndexFor {
            label: "Person".to_owned(),
            prop: "email".to_owned(),
        };
    ...
    fn create_unique_constraint_adds_to_catalog() {
        let g = make_graph();
        let stmt = tessera_graph::gql::DdlStatement::CreateUniqueConstraint {
    ...
    fn show_index_info_returns_tabular_rows() {
        ...
        let result = dispatch_ddl(tessera_graph::gql::DdlStatement::ShowIndexInfo, &g).unwrap();
    ...
    fn show_constraint_info_returns_tabular_rows() {
        ...
        g.write()
            ...
            .add_unique_constraint("Asset", "id");
        let result =
            dispatch_ddl(tessera_graph::gql::DdlStatement::ShowConstraintInfo, &g).unwrap();
        assert!(!result.fields_psv.is_empty());
    ...
    fn create_index_persists_schema_bin() {
        ...
        {
            let graph = tessera_graph::Graph::open(&path, &cfg).unwrap();
        ...
        {
            let g = tessera_graph::Graph::open(&path, &cfg).unwrap();
            assert!(
