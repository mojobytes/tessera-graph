...
impl SingleDatabaseManager {
    ...
    pub async fn new(
        ...
        // feeds the `tessera_wal_fsync_duration_seconds` histogram, so
        // Community gets the same durability metrics as Enterprise.
        let wal_observer: tessera_graph::WalObserver = Box::new(crate::metrics::wal_fsync_observed);
        ...
        let interval = if interval.is_zero() {
            tracing::warn!(
                "vacuum interval=0 reached spawn (should be gated off upstream); \
                 ...
                 TESSERA_VACUUM_INTERVAL_SECONDS to a positive value."
            );
            MIN_SWEEP_INTERVAL
        } else {
            interval
