...
use tessera_graph::gql::param_substitution::{self, ParamError};
    ...
    A: AuthProvider,
{
    ...
    ///
    /// `tessera_bolt_messages_total{type, outcome}` is emitted exactly
    ...
    async fn handle_run(
        ...
        let params_signature = tessera_graph_cypher::cache::hash_params(&param_map);
        let mut stmt = match tessera_graph_cypher::parse_with_mode_cached(
            query,
        ...
        let exec_result: std::result::Result<PendingResult, String> = match stmt {
            ...
            GqlStatement::Pipeline(ref pq) => {
                ...
                let cols: Vec<String> = match &pq.terminal {
                    tessera_graph::gql::PipelineTerminal::Return { clause, .. } => {
        ...
        match exec_result {
            ...
            Err(msg) => {
                self.failed = true;
                // Task 15: a `tessera_graph::Error::QuotaExceeded`
    ...
    async fn dispatch_ddl(&mut self, stmt: tessera_graph::gql::DdlStatement) -> Result<bool> {
    ...
    fn dispatch_const_return(
        &self,
        c: &tessera_graph::gql::ConstReturnQuery,
        params: HashMap<String, tessera_graph::gql::GqlValue>,
    ) -> std::result::Result<Vec<crate::graph_accessor::ResultRow>, String> {
        let deadline = self.compute_deadline();
        match self.open_txn {
...
/// `crates/tessera-graph-server/src/auth/system_graph.rs` (intentionally
...
fn ast_is_mutating(stmt: &tessera_graph::gql::GqlStatement) -> bool {
...
fn mutation_stats_to_dict(stats: &tessera_graph::gql::GqlMutationResult) -> PackStreamValue {
...
fn return_items_columns(items: &[tessera_graph::gql::ReturnItem]) -> Vec<String> {
    items
        .iter()
        .map(|it| {
            it.alias
                .clone()
                .unwrap_or_else(|| tessera_graph::gql::expr_surface_name(&it.expr))
        })
        .collect()
}
