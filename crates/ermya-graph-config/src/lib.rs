...
//! Configuration management for ermya-graph-enterprise.
...
impl AuditConfig {
    ...
    ///
    /// - `ERMYA_AUDIT_ENABLED` (default `"true"`)
    /// - `ERMYA_AUDIT_PATH` (default `"audit.ndjson"`)
    /// - `ERMYA_AUDIT_ROTATION_MAX_MB` (default `100`)
    /// - `ERMYA_AUDIT_MAX_FILES` (default `0` = keep all)
    #[must_use]
    pub fn from_env() -> Self {
        let enabled =
            std::env::var("ERMYA_AUDIT_ENABLED").map_or(true, |v| v != "false" && v != "0");

        let log_path = std::env::var("ERMYA_AUDIT_PATH").map_or_else(
            |_| std::path::PathBuf::from("audit.ndjson"),
            std::path::PathBuf::from,
        );

        let rotation_max_mb: u64 = std::env::var("ERMYA_AUDIT_ROTATION_MAX_MB")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(100);
        let rotation_max_size_bytes = rotation_max_mb * 1024 * 1024;

        let max_rotated_files: usize = std::env::var("ERMYA_AUDIT_MAX_FILES")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(0);

        let channel_capacity: usize = std::env::var("ERMYA_AUDIT_CHANNEL_CAPACITY")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(4096);
