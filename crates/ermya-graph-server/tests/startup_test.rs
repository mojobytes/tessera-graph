...
mod tests {
    ...
    use ermya_graph_server::migration::CURRENT_DISK_LAYOUT;
    #[cfg(unix)]
    use ermya_graph_server::migration::write;
    ...
    async fn start_server_binds_and_shuts_down() {
        ...
        // `/var/lib/ermya/data` (root-only on a dev host). Without this the
    ...
    async fn startup_stamps_version_on_fresh_data_dir() {
        let dir = tempfile::tempdir().unwrap();
        // Genuinely empty data_dir: no `system/`, no `.ermya-version`.
        ...
        let marker = dir.path().join(".ermya-version");
        assert!(
            marker.exists(),
            "fresh-install startup must stamp `.ermya-version`"
        );
    ...
    async fn startup_rejects_populated_dir_without_marker() {
        ...
        // but no `.ermya-version`. The fresh-install heuristic must
    ...
    async fn startup_refuses_world_readable_system_dir() {
        ...
        let (_tx, rx) = tokio::sync::watch::channel(false);
        let err = ermya_graph_server::start_server_with_registry(
            cfg,
            rx,
            None,
            ermya_graph_server::single_database_factory(),
    ...
    async fn startup_respects_no_auth_env_var() {
        ...
        // `/var/lib/ermya/data`, which is root-only on a dev host. Isolate
    ...
    async fn start_server_reports_bound_addr_via_oneshot() {
        ...
        // right after `ErmyaListener::bind` and before entering the accept
    ...
    async fn start_server_with_password_rejects_bad_credentials() {
        ...
        tokio::spawn(async move {
            ...
            let listener = ermya_graph_server::ErmyaListener::bind(&config.bind_addr)
                .await
                .unwrap();
            let addr = listener.local_addr().unwrap();
            let _ = addr_tx.send(addr);

            let auth: std::sync::Arc<dyn ermya_graph_server::auth::AuthProvider> =
                std::sync::Arc::new(ermya_graph_server::auth::PasswordAuthProvider::new(
                    "correct-pw",
                ));
            let graph = std::sync::Arc::new(ermya_graph_server::DefaultGraphAccessor::new(
                std::sync::Arc::new(std::sync::RwLock::new(ermya_graph::Graph::new())),
            ));
        ...
        let hello = BoltRequest::Hello {
            extra: vec![
                ...
                (
                    "credentials".to_owned(),
                    ermya_graph_protocol::PackStreamValue::String("wrong-pw".to_owned()),
                ),
            ],
        };
    ...
    async fn start_server_with_password_accepts_correct_credentials() {
        ...
        tokio::spawn(async move {
            let listener = ermya_graph_server::ErmyaListener::bind("127.0.0.1:0")
                .await
                .unwrap();
            let addr = listener.local_addr().unwrap();
            let _ = addr_tx.send(addr);
        ...
        let hello = BoltRequest::Hello {
            extra: vec![
                ...
                (
                    "credentials".to_owned(),
                    ermya_graph_protocol::PackStreamValue::String("correct-pw".to_owned()),
                ),
            ],
        };
        let data = encode_request(&hello).unwrap();
    ...
    // clean (no ERMYA_DATA_DIR) now tries to write the data dir. If that path
    ...
    // (ERMYA_DATA_DIR and the :memory: sentinel) — never a cryptic OS error.
    #[cfg(unix)]
    #[tokio::test]
    async fn start_server_fails_with_actionable_error_on_unwritable_data_dir() {
        ...
        let (_tx, rx) = tokio::sync::watch::channel(false);
        let result = ermya_graph_server::start_server_with_registry(
            cfg,
            rx,
            None,
            ermya_graph_server::single_database_factory(),
        ...
        assert!(
            msg.contains("ermya_data_dir"),
            "error must mention ERMYA_DATA_DIR; got: {msg}"
        );
        assert!(
            msg.contains(":memory:"),
