...
use ermya_graph_server::auth::{AuthError, AuthOutcome, AuthProvider, NoAuthProvider};
...
fn auth_outcome_exposes_is_admin_field() {
    let outcome = ermya_graph_server::auth::AuthOutcome {
        user_id: "u".into(),
        roles: vec![],
        is_admin: true,
    };
...
fn auth_outcome_default_is_admin_is_false_for_noop() {
    use ermya_graph_server::auth::{AuthProvider, NoAuthProvider};
