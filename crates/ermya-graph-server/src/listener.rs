...
//!
//! [`ErmyaListener`] provides:
//! - [`serve_with`](ErmyaListener::serve_with): generic accept loop — the
//!   extension point for enterprise to plug in its own handler.
//! - [`serve_plain`](ErmyaListener::serve_plain): plain TCP for development
//!   and testing (feature-gated behind `plain-tcp`).
//! - [`serve_tls`](ErmyaListener::serve_tls): TLS-enforced for production.
...
///
/// On rejection, emits `ermya_rate_limit_hits_total{axis="conn_ip"}`
...
impl ErmyaListener {
    ...
    {
        let query_cache = Arc::new(ermya_graph_cypher::cache::QueryCache::new(4096));
