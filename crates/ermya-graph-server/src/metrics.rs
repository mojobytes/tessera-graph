...
pub(crate) const ACTIVE_CONNECTIONS: &str = "ermya_active_connections";
pub(crate) const BOLT_MESSAGES_TOTAL: &str = "ermya_bolt_messages_total";
pub(crate) const AUTH_ATTEMPTS_TOTAL: &str = "ermya_auth_attempts_total";
pub(crate) const QUERIES_TOTAL: &str = "ermya_queries_total";
pub(crate) const QUERY_DURATION_SECONDS: &str = "ermya_query_duration_seconds";
...
/// not recorded — see `ermya_graph::Graph::wal_sync`). No labels:
...
pub(crate) const WAL_FSYNC_DURATION_SECONDS: &str = "ermya_wal_fsync_duration_seconds";
...
pub(crate) const RESULT_CAPPED_TOTAL: &str = "ermya_result_capped_total";
...
pub(crate) const RATE_LIMIT_HITS_TOTAL: &str = "ermya_rate_limit_hits_total";
...
pub(crate) const QUERY_TIMEOUTS_TOTAL: &str = "ermya_query_timeouts_total";
...
/// Increment the `ermya_active_connections` gauge by 1.
///
...
/// Decrement the `ermya_active_connections` gauge by 1.
///
...
/// Increment `ermya_bolt_messages_total{type, outcome}` by 1.
///
...
/// Increment `ermya_auth_attempts_total{outcome}` by 1.
///
...
/// Increment `ermya_queries_total{database, outcome}` by 1.
///
...
/// Increment `ermya_result_capped_total{database}` by 1.
///
...
/// v0.6.0 Fase 2 Task 6 — increment `ermya_query_timeouts_total{database}`
...
/// Record `ermya_query_duration_seconds{database, kind}` with the
...
///
/// [`GqlStatement`]: ermya_graph::gql::GqlStatement
...
/// in Task 1 C6.
pub(crate) fn wal_fsync_observed(_cause: ermya_graph::FsyncCause, duration: std::time::Duration) {
...
///
/// [`GqlStatement`]: ermya_graph::gql::GqlStatement
pub(crate) const fn gql_statement_kind(stmt: &ermya_graph::gql::GqlStatement) -> &'static str {
    use ermya_graph::gql::GqlStatement as S;
    match stmt {
        S::Query(_) => "query",
...
pub(crate) const fn bolt_request_type_str(
    req: &ermya_graph_protocol::bolt_message::BoltRequest,
) -> &'static str {
    use ermya_graph_protocol::bolt_message::BoltRequest as R;
    match req {
        R::Hello { .. } => "HELLO",
        R::Logon { .. } => "LOGON",
        R::Run { .. } => "RUN",
...
mod tests {
    ...
    fn wal_fsync_duration_seconds_constant_matches_metric_name() {
        ...
        assert_eq!(
            ...
            "ermya_wal_fsync_duration_seconds",
        );
    }
