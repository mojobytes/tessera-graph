...
pub enum MigrationError {
    #[error(
        "disk layout out of date; run 'ermya-graph-cli migrate' first \
...
/// Read `.ermya-version` from `data_dir` and reject any layout
...
/// Write a fresh `.ermya-version` with the current layout iff the
...
/// Write a fresh `.ermya-version` with the current layout. Used by
...
pub(super) const VERSION_FILE: &str = ".ermya-version";
...
///   inspectable by `ermya-graph-cli migrate --dry-run`, which is
