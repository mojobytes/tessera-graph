...
//!    via `ERMYA_NO_AUTH=1`; otherwise `SystemGraphAuthProvider` on top
...
use crate::listener::ErmyaListener;
use crate::migration;
    ...
    Arc::new(|config, system_store| {
        Box::pin(async move {
            ...
            let data_dir = config.data_dir.clone().unwrap_or_else(|| {
                std::env::temp_dir().join(format!("ermya-inmem-{}", std::process::id()))
            });
...
async fn start_server_inner(
    ...
    // The registry-derived gauges (`ermya_open_databases`,
    // `ermya_database_size_bytes`) report per-database state, which only a
    ...
    let listener = ErmyaListener::bind(&config.bind_addr).await?;
    ...
    if config.tls_cert.is_none() || config.tls_key.is_none() {
        return Err(ServerError::Io(std::io::Error::new(
            ...
            "TLS certificate and key are required \
             (set ERMYA_TLS_CERT and ERMYA_TLS_KEY)",
        )));
    }
...
async fn bind_metrics_endpoint(
    ...
    let parsed: SocketAddr = raw.parse().map_err(|e: std::net::AddrParseError| {
        ServerError::Io(std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            format!("ERMYA_METRICS_ADDR={raw:?} is not a valid socket address: {e}"),
        ))
    })?;
    let bound = crate::metrics::spawn_metrics_server(parsed, shutdown)
        .await
        .map_err(ServerError::Io)?;
    // Match the unauth warn ERMYA_NO_AUTH already emits at startup:
...
fn ensure_data_dir_prepared(data_dir: &Path) -> Result<()> {
    ...
    let probe = data_dir.join(".ermya-write-probe");
...
async fn build_runtime(
    ...
    // Persistent deployments must carry a `.ermya-version` marker
    ...
    if let Some(ref data_dir) = config.data_dir {
        ...
        let marker_path = data_dir.join(".ermya-version");
    ...
    let auth: Arc<dyn AuthProvider> = if config.no_auth {
        tracing::warn!(
            "ERMYA_NO_AUTH=1: authentication disabled. \
...
fn build_audit_sink(config: &ServerConfig, shutdown: watch::Receiver<bool>) -> Result<AuditSink> {
    match config.audit_sink {
        AuditSinkKind::Off => {
            tracing::warn!(
                "ERMYA_AUDIT_SINK=off: audit log disabled. \
...
fn resolve_audit_path(config: &ServerConfig) -> Result<PathBuf> {
    ...
    Err(ServerError::Io(std::io::Error::new(
        std::io::ErrorKind::InvalidInput,
        "ERMYA_AUDIT_SINK=file requires ERMYA_AUDIT_FILE or ERMYA_DATA_DIR",
    )))
}
...
fn open_system_graph(
    ...
    let Some(ref data_dir) = config.data_dir else {
        tracing::warn!(
            "no ERMYA_DATA_DIR configured: system graph is in-memory. \
    ...
    let graph_config = ermya_graph::GraphConfig::new();
...
/// When the operator has supplied `ERMYA_PASSWORD` we use it and emit
...
async fn bootstrap_admin_if_empty(
    ...
    if !existing.is_empty() {
        if config.password.is_some() {
            tracing::warn!(
                "ERMYA_PASSWORD set but ignored: system graph already has \
