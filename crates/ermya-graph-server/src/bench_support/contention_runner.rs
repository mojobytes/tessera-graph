...
use ermya_graph::gql::{self, GqlValue};
...
fn reader_match_clause() -> gql::MatchClause {
    let stmt = ermya_graph_cypher::parse_with_mode(
...
pub(crate) fn mutation_for_scenario(scenario: Scenario) -> gql::MutationStatement {
    ...
    let stmt = ermya_graph_cypher::parse_with_mode(cypher, QueryLanguage::CypherCompat)
