...
pub fn execute(py: Python<'_>, graph: &PyGraph, query: &str) -> PyResult<PyGqlResult> {
    let parsed = tessera_graph::gql::parse(query).map_err(to_py_err)?;
    ...
    let rows = tessera_graph::gql::execute(&graph.inner, &parsed, 0).map_err(to_py_err)?;
...
pub fn validate(query: &str) -> PyResult<bool> {
    tessera_graph::gql::parse_statement(query).map_err(to_py_err)?;
    Ok(true)
}
...
impl PyGqlResult {
    ...
    // TODO: populate when mutation executor is added to tessera-graph
    #[getter]
