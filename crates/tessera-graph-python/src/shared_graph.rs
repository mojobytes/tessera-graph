...
fn read_lock(lock: &RwLock<Graph>) -> PyResult<RwLockReadGuard<'_, Graph>> {
    lock.read()
        .map_err(|_| errors::TesseraError::new_err("shared graph lock is poisoned"))
}
...
macro_rules! impl_shared_read_methods {
    ($ty:ident) => {
        #[pymethods]
        impl $ty {
            fn node_count(&self) -> PyResult<usize> { Ok(read_lock(&self.inner)?.node_count()) }
            fn edge_count(&self) -> PyResult<usize> { Ok(read_lock(&self.inner)?.edge_count()) }
            ...
            fn node(&self, id: &PyNodeId) -> PyResult<PyNode> {
                read_lock(&self.inner)?.node(to_node_id(id)).map(PyNode::from).map_err(to_py_err)
            }

            fn node_ids(&self) -> PyResult<Vec<PyNodeId>> {
                Ok(read_lock(&self.inner)?.node_ids().into_iter().map(PyNodeId::from).collect())
            }

            fn nodes_by_label(&self, label: &str) -> PyResult<Vec<PyNodeId>> {
                Ok(read_lock(&self.inner)?.nodes_by_label(label).into_iter().map(PyNodeId::from).collect())
            }

            fn edge(&self, id: &PyEdgeId) -> PyResult<PyEdge> {
                read_lock(&self.inner)?.edge(to_edge_id(id)).map(PyEdge::from).map_err(to_py_err)
            }

            fn edges_by_label(&self, label: &str) -> PyResult<Vec<PyEdgeId>> {
                Ok(read_lock(&self.inner)?.edges_by_label(label).into_iter().map(PyEdgeId::from).collect())
            }

            fn outgoing_edges(&self, node: &PyNodeId) -> PyResult<Vec<PyEdge>> {
                read_lock(&self.inner)?.outgoing_edges(to_node_id(node))
                    .map(|v| v.into_iter().map(PyEdge::from).collect())
                    .map_err(to_py_err)
            }

            fn incoming_edges(&self, node: &PyNodeId) -> PyResult<Vec<PyEdge>> {
                read_lock(&self.inner)?.incoming_edges(to_node_id(node))
                    .map(|v| v.into_iter().map(PyEdge::from).collect())
                    .map_err(to_py_err)
            }
        }
...
impl PySharedGraph {
    ...
    fn new_(graph: &mut PyGraph) -> Self {
        ...
        Self { inner: Arc::new(RwLock::new(taken)) }
    }

    fn read(&self) -> PyReadGuard { PyReadGuard { inner: Arc::clone(&self.inner) } }
    fn write(&self) -> PyWriteGuard { PyWriteGuard { inner: Arc::clone(&self.inner) } }

    fn __repr__(&self) -> PyResult<String> {
        let g = read_lock(&self.inner)?;
        Ok(format!("SharedGraph(nodes={}, edges={})", g.node_count(), g.edge_count()))
    }
}
...
pub struct PyReadGuard { inner: Arc<RwLock<Graph>> }
...
impl PyReadGuard {
    ...
    fn __exit__(&self, _exc_type: Option<&Bound<'_, PyAny>>, _exc_val: Option<&Bound<'_, PyAny>>, _exc_tb: Option<&Bound<'_, PyAny>>) -> bool { false }
}

// ── WriteGuard ──────────────────────────────────────────────────────────────

#[pyclass(name = "WriteGuard")]
pub struct PyWriteGuard { inner: Arc<RwLock<Graph>> }
...
impl PyWriteGuard {
    ...
    fn add_node(&self, label: &str, properties: &Bound<'_, PyDict>) -> PyResult<PyNodeId> {
        ...
        write_lock(&self.inner)?.add_node(label, props).map(PyNodeId::from).map_err(to_py_err)
    }

    fn add_edge(&self, label: &str, source: &PyNodeId, target: &PyNodeId, properties: &Bound<'_, PyDict>) -> PyResult<PyEdgeId> {
        let props = properties::from_py_dict(properties)?;
        write_lock(&self.inner)?.add_edge(label, to_node_id(source), to_node_id(target), props).map(PyEdgeId::from).map_err(to_py_err)
    }

    fn remove_node(&self, id: &PyNodeId) -> PyResult<PyNode> {
        write_lock(&self.inner)?.remove_node(to_node_id(id)).map(PyNode::from).map_err(to_py_err)
    }

    fn remove_edge(&self, id: &PyEdgeId) -> PyResult<PyEdge> {
        write_lock(&self.inner)?.remove_edge(to_edge_id(id)).map(PyEdge::from).map_err(to_py_err)
    }

    #[pyo3(signature = (id, *, label=None, properties=None))]
    fn update_node(&self, id: &PyNodeId, label: Option<&str>, properties: Option<&Bound<'_, PyDict>>) -> PyResult<()> {
        let rust_id = to_node_id(id);
        let mut g = write_lock(&self.inner)?;
        let mut node = g.node(rust_id).map_err(to_py_err)?;
        if let Some(l) = label { node.set_label(l); }
        if let Some(dict) = properties { *node.properties_mut() = properties::from_py_dict(dict)?; }
        g.update_node(rust_id, &node).map_err(to_py_err)
    }

    #[pyo3(signature = (id, *, label=None, properties=None))]
    fn update_edge(&self, id: &PyEdgeId, label: Option<&str>, properties: Option<&Bound<'_, PyDict>>) -> PyResult<()> {
        let rust_id = to_edge_id(id);
        let mut g = write_lock(&self.inner)?;
        let mut edge = g.edge(rust_id).map_err(to_py_err)?;
        if let Some(l) = label { edge.set_label(l); }
        if let Some(dict) = properties { *edge.properties_mut() = properties::from_py_dict(dict)?; }
