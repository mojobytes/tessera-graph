...
/// Converts `PyNodeId` → `tessera_graph::NodeId`.
pub(crate) fn to_node_id(py_id: &PyNodeId) -> tessera_graph::NodeId {
    tessera_graph::NodeId::from_raw(py_id.value)
}

/// Converts `PyEdgeId` → `tessera_graph::EdgeId`.
pub(crate) fn to_edge_id(py_id: &PyEdgeId) -> tessera_graph::EdgeId {
    tessera_graph::EdgeId::from_raw(py_id.value)
}
