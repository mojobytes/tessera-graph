...
def test_graph_new_returns_graph_object():
    from tessera_graph import Graph
...
def test_graph_open_raises_tessera_error_when_dir_missing_and_flag_false():
    from tessera_graph import Graph, TesseraError
...
def test_exception_hierarchy():
    ...
    assert issubclass(NodeNotFoundError, TesseraError)
    assert issubclass(EdgeNotFoundError, TesseraError)
    assert issubclass(GqlSyntaxError, TesseraError)
    assert issubclass(TesseraError, Exception)


def test_node_not_found_is_catchable_as_tessera_error():
    from tessera_graph import Graph, NodeId, TesseraError, NodeNotFoundError
...
def test_node_id_wraps_u64():
    from tessera_graph import NodeId
...
def test_edge_id_wraps_u64():
    from tessera_graph import EdgeId
