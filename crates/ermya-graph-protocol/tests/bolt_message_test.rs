...
use ermya_graph_protocol::packstream::decode as ps_decode;
...
fn sample_dict() -> BoltDict {
    vec![
        (
            "user_agent".to_owned(),
            PackStreamValue::String("ermya/1.0".to_owned()),
        ),
        (
            "scheme".to_owned(),
...
// NOTE for kripteia: the assertion that validates roundtrip correctness
// lives inside this helper. Tests that call `assert_request_roundtrip`
// (e.g. `roundtrip_hello`, `roundtrip_logon`) delegate equality checking
// here, so static analysis tools that only scan the test body itself may
// report "no assertions" — those are false positives.
...
fn decode_request_unknown_tag_returns_unexpected_tag_error() {
    ...
    use ermya_graph_protocol::packstream::encode as ps_encode;
...
fn decode_request_non_struct_returns_unexpected_tag_error() {
    ...
    let err = decode_request(&buf).unwrap_err();
    assert!(
        matches!(err, ProtocolError::BoltNotAStruct),
        "expected BoltNotAStruct for non-struct input, got: {err:?}"
    );
}
