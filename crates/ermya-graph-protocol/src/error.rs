...
pub enum ProtocolError {
    ...
    #[error("bolt: expected a PackStream struct but got a different value type")]
    BoltNotAStruct,
    ...
    #[error("bolt: unexpected {got} response to {request}")]
    BoltUnexpectedResponse {
        request: &'static str,
        got: &'static str,
    },

    #[error("bolt: RESET failed — {message}")]
    BoltResetFailure { message: String },

    #[error("bolt: request ignored — connection is in FAILED state (send RESET to recover)")]
    BoltConnectionIgnored,
}
