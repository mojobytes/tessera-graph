...
use tessera_graph_benchmark::error::{BenchmarkError, Result};
use tessera_graph_benchmark::report::{Report, ReportEntry};
use tessera_graph_benchmark::scenario::{
...
pub fn run_scenario(args: &CliArgs) -> Result<Report> {
    match args.target {
        ...
        Target::TesseraBolt => {
            let mut target =
                tessera_graph_benchmark::tessera_bolt_target::TesseraBoltTarget::connect(
        ...
        #[cfg(feature = "memgraph")]
        Target::Memgraph => {
            let mut target = tessera_graph_benchmark::memgraph_target::MemgraphTarget::connect(
                &args.memgraph_uri,
                &args.memgraph_user,
                &args.memgraph_pass,
