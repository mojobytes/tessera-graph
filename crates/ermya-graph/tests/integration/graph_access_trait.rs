...
fn graph_impl_satisfies_trait() {
    fn use_trait<G: GraphAccess>(g: &mut G) -> ermya_graph::NodeId {
...
fn graph_access_edge_operations() {
    fn edge_ops<G: GraphAccess>(g: &mut G, src: ermya_graph::NodeId, dst: ermya_graph::NodeId) {
...
fn gql_compiler_works_with_delegating_graph() {
    ...
    let query = ermya_graph::gql::parse("MATCH (a:Person) RETURN a.name").unwrap();
    let result = ermya_graph::gql::execute(&proxy, &query, 0).unwrap();

    assert_eq!(result.len(), 1);
    let row = &result[0];
    assert_eq!(
        row.get("a.name"),
        Some(&ermya_graph::GqlValue::Str("Alice".into()))
    );
}
...
fn existing_graph_sugar_api_unchanged() {
    ...
    // GQL via Graph
    let query = ermya_graph::gql::parse("MATCH (a:A) RETURN a.name").unwrap();
    let _result = ermya_graph::gql::execute(&g, &query, 0).unwrap();
}
...
fn gql_two_anonymous_nodes_do_not_collide() {
    ...
    let query = ermya_graph::gql::parse("MATCH (:Person)-[:KNOWS]->(:Person) RETURN 1").unwrap();
    let result = ermya_graph::gql::execute(&g, &query, 0).unwrap();
