...
fn run(
    graph: &Graph,
    query: &str,
) -> ermya_graph::Result<Vec<std::collections::HashMap<String, GqlValue>>> {
...
fn compile_match_unknown_var_in_return_is_compile_error() {
    ...
    assert!(matches!(err, ermya_graph::Error::GqlCompileError(_)));
}
...
fn create_with_literal_props_still_works() {
    use ermya_graph::gql::{CreatePattern, Expr, GqlStatement, Literal, MutationClause};
    ...
    let mut g = Graph::new();
    let props = ermya_graph::props! { "val" => 42_i64, "name" => "test" };
...
fn unwind_create_node_with_variable_prop() {
    use ermya_graph::gql::{self, GqlStatement};
    ...
    // unwind mutation inline via the public API from ermya-graph.
    {
        let graph = shared.read().unwrap();
        let empty_pm = ermya_graph::PatternMatch::empty();
...
fn resolve_create_props_literal_only() {
    use ermya_graph::gql::{self, Expr, Literal};
    ...
    assert_eq!(result.get("age"), Some(&ermya_graph::Property::I64(30)));
    assert_eq!(
        result.get("active"),
        Some(&ermya_graph::Property::Bool(true))
    );
