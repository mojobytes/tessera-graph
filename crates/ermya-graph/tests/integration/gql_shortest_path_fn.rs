...
fn shortest_path_two_connected_nodes_returns_list() {
    ...
    match val {
        ermya_graph::gql::GqlValue::List(ids) => {
...
fn shortest_path_unreachable_returns_null() {
    ...
    assert_eq!(rows.len(), 1);
    let val = rows[0].get("shortestpath(a, b)").expect("column exists");
    assert_eq!(*val, ermya_graph::gql::GqlValue::Null);
}
...
fn shortest_path_unbound_variable_returns_compile_error() {
    let g = Graph::new();
    let query = ermya_graph::gql::parse("MATCH (a:P) RETURN shortestPath(a, z)").unwrap();
    let result = ermya_graph::gql::execute(&g, &query, 0);
