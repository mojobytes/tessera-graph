...
use ermya_graph::GqlQuery;
use ermya_graph::gql;
use ermya_graph::gql::{BinOp, CreatePattern, Expr, GqlStatement, Literal, MutationClause};
...
fn parse_multi_label_node_is_unsupported_error() {
    ...
    assert!(matches!(err, ermya_graph::Error::GqlUnsupported(_)));
}
...
fn parse_deeply_nested_aggregate_is_rejected() {
    ...
    assert!(
        matches!(err, ermya_graph::Error::GqlSyntaxError { .. }),
