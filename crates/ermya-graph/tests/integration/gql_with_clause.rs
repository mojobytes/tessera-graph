...
fn with_is_tokenized_as_keyword_not_identifier() {
    use ermya_graph::gql::token::Token;

    // Lexer produces a dedicated Token::With variant.
    let tokens = ermya_graph::gql::lexer::Lexer::new("WITH")
        .tokenize()
        .unwrap();
    assert!(
    ...
    // Case-insensitive.
    let tokens_lc = ermya_graph::gql::lexer::Lexer::new("with")
        .tokenize()
        .unwrap();
    assert!(matches!(
...
fn expr_subscript_and_list_lit_constructible() {
    use ermya_graph::gql::{Expr, Literal};
...
fn parse_simple_match_with_return_produces_pipeline() {
    use ermya_graph::gql::{GqlStatement, PipelineStage, PipelineTerminal};

    let stmt = ermya_graph::gql::parse_statement("MATCH (a) WITH a RETURN a").unwrap();
...
fn parse_with_full_options() {
    ...
    let stmt = ermya_graph::gql::parse_statement(
        "MATCH (a) WITH a WHERE a.age > 30 ORDER BY a.id SKIP 1 LIMIT 10 RETURN a",
    )
    .unwrap();
    let GqlStatement::Pipeline(pq) = stmt else {
...
fn parse_query_without_with_stays_flat() {
    ...
    let stmt = ermya_graph::gql::parse_statement("MATCH (a:Person) RETURN a").unwrap();
    assert!(
...
fn parse_list_subscript_in_return() {
    use ermya_graph::gql::{Expr, GqlStatement};

    let stmt =
        ermya_graph::gql::parse_statement("MATCH (a) WITH [10, 20, 30] AS lst RETURN lst[1]")
            .unwrap();
    let GqlStatement::Pipeline(pq) = stmt else {
        panic!("expected Pipeline");
    };
    let ermya_graph::gql::PipelineTerminal::Return { clause, .. } = &pq.terminal else {
...
fn parse_list_literal_homogeneous_vs_heterogeneous() {
    use ermya_graph::gql::{Expr, GqlStatement, Literal};

    // All-literal: keeps `Literal::List` — backwards-compatible.
    let stmt =
        ermya_graph::gql::parse_statement("MATCH (a) WITH [1, 2, 3] AS xs RETURN xs").unwrap();
    let GqlStatement::Pipeline(pq) = stmt else {
        panic!();
    };
    let ermya_graph::gql::PipelineStage::With(w) = &pq.stages[1] else {
        panic!();
    };
    ...
    let stmt =
        ermya_graph::gql::parse_statement("MATCH (a) WITH [1, a, 3] AS xs RETURN xs").unwrap();
    let GqlStatement::Pipeline(pq) = stmt else {
        panic!();
    };
    let ermya_graph::gql::PipelineStage::With(w) = &pq.stages[1] else {
...
fn execute_simple_passthrough_pipeline() {
    ...
    let stmt = ermya_graph::gql::parse_statement("MATCH (a:Person) WITH a RETURN a").unwrap();
    let ermya_graph::gql::GqlStatement::Pipeline(ref pq) = stmt else {
        panic!("expected Pipeline");
    };
    let rows = ermya_graph::gql::execute_pipeline(&g, pq, 0).unwrap();
...
fn ages_graph() -> ermya_graph::Graph {
    use ermya_graph::{Graph, props};
    let mut g = Graph::new();
    g.add_node(
        "Person",
...
fn execute_with_property_projection() {
    ...
    let g = ages_graph();
    let stmt =
        ermya_graph::gql::parse_statement("MATCH (a:Person) WITH a.name AS name RETURN name")
            .unwrap();
    let ermya_graph::gql::GqlStatement::Pipeline(ref pq) = stmt else {
        panic!();
    };
...
fn execute_with_where_filter() {
    let g = ages_graph();
    let stmt =
        ermya_graph::gql::parse_statement("MATCH (a:Person) WITH a WHERE a.age > 30 RETURN a")
            .unwrap();
    let ermya_graph::gql::GqlStatement::Pipeline(ref pq) = stmt else {
        panic!();
    };
...
fn execute_with_limit() {
    let g = ages_graph();
    let stmt =
        ermya_graph::gql::parse_statement("MATCH (a:Person) WITH a LIMIT 2 RETURN a").unwrap();
    let ermya_graph::gql::GqlStatement::Pipeline(ref pq) = stmt else {
        panic!();
    };
    let rows = ermya_graph::gql::execute_pipeline(&g, pq, 0).unwrap();
...
fn execute_with_collect_list_contents() {
    ...
    let g = ages_graph();
    let stmt =
        ermya_graph::gql::parse_statement("MATCH (a:Person) WITH collect(a) AS all RETURN all")
            .unwrap();
    let ermya_graph::gql::GqlStatement::Pipeline(ref pq) = stmt else {
        panic!();
    };
...
fn execute_scope_isolation_after_with() {
    ...
    let err = ermya_graph::gql::execute_pipeline(&g, pq, 0)
...
fn execute_range_positive() {
    use ermya_graph::{GqlValue, Graph, props};
...
fn execute_range_at_cap_boundary() {
    ...
    let stmt =
        ermya_graph::gql::parse_statement("MATCH (a:Person) WITH range(0, 999999) AS r RETURN r")
            .unwrap();
    let ermya_graph::gql::GqlStatement::Pipeline(ref pq) = stmt else {
        panic!();
    };
...
fn execute_subscript_out_of_bounds_returns_null() {
    ...
    let stmt =
        ermya_graph::gql::parse_statement("MATCH (a:Person) WITH [10, 20] AS lst RETURN lst[5]")
            .unwrap();
    let ermya_graph::gql::GqlStatement::Pipeline(ref pq) = stmt else {
        panic!();
    };
...
fn execute_subscript_in_legacy_match_return() {
    use ermya_graph::{GqlValue, Graph, gql, props};
...
fn execute_unwind_null_drops_binding() {
    ...
    let stmt =
        ermya_graph::gql::parse_statement("MATCH (a:Person) WITH a UNWIND null AS n RETURN n")
            .unwrap();
    let ermya_graph::gql::GqlStatement::Pipeline(ref pq) = stmt else {
        panic!();
    };
...
fn execute_with_set_unknown_variable_is_error() {
    use ermya_graph::{Graph, gql, props};
...
fn execute_pipeline_set_in_txn_writes_pending_not_autocommit() {
    ...
    assert_eq!(
        ...
        Some(&ermya_graph::Property::I64(42))
    );
