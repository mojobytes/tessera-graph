...
///
/// This is the core data structure of `ErmyaGraph`. It stores nodes and
...
/// `ermya-graph-server` crate uses in v0.6.0 Task 2 to feed the
/// `ermya_wal_fsync_duration_seconds` Prometheus histogram without
...
impl Graph {
    ...
    /// `CALL ermya.vertex_labels()` / `mg.vertex_labels()`. Allocates on every
    ...
    /// `CALL ermya.edge_types()` / `mg.edge_types()`. Allocates on every call;
...
///
/// ```rust
/// # use ermya_graph::{SharedGraph, Graph, props};
...
mod tests {
    ...
    // 3d T1: introspection getters for CALL ermya.vertex_labels/edge_types.
    #[test]
    ...
    ///
    ///     cargo test -p ermya-graph --lib --release --features pool-instrumentation \
