...
//! Supported namespaces: `ermya` (canonical) and `mg` (Memgraph alias for
//! query compatibility).
//!
...
pub enum ProcedureKind {
    ...
    EdgeTypes,
    /// `ermya.snapshot(db, dest)` — administrative physical snapshot. No `mg`
    /// alias (Memgraph has no equivalent). Execution is delegated to the host.
    Snapshot,
    /// `ermya.restore(db, snap)` — administrative restore from a snapshot. No
