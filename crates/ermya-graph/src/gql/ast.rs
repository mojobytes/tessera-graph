...
pub enum GqlStatement {
    ...
    /// `ermya-graph-cypher` and executed by the server's admin handler;
    ...
    /// `ermya-graph-cypher` and executed by the server's DDL handler;
    /// the core engine exposes only the schema catalog API.
    Ddl(DdlStatement),
    /// A `CALL <proc>() YIELD <col> [UNWIND …] [RETURN …]` statement (built-in
    /// procedure invocation). Parsed by `ermya-graph-cypher` and executed by
...
pub enum AdminStatement {
    ...
    ///
    /// Parsed by `ermya-graph-cypher`, executed by the server admin
...
///
/// Parsed by `ermya-graph-cypher` and executed by the server's DDL
...
/// `ErmyaGraph`'s [`crate::PropertyIndex`] is a TOTAL index: every
...
pub struct CallStatement {
    /// The namespace component before the dot, e.g. `"mg"` or `"ermya"`.
    ...
    /// literals of `ermya.snapshot('mydb', '/dest')`. Empty for the
...
///
/// Subset of `ermya_graph_server::auth::AccessLevel` — `None` is
...
///
/// Kept distinct from `ermya_graph_server::auth::DatabaseOptions` so
