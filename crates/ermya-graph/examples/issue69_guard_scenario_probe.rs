...
//!
//! - `ERMYA_DISABLE_PUSHDOWN` — aggregate pushdown, in `gql::compiler`
//! - `ERMYA_DISABLE_INTERSECTION` — multi-index intersection, in `query::pattern`
//! - `ERMYA_DISABLE_LABEL_FASTPATH` — label-only fast path, in `query::pattern`
//!
//! Run the baseline, then one arm per optimization:
//!
//! ```text
//! cargo run --example issue69_guard_scenario_probe -p ermya-graph
//! ERMYA_DISABLE_PUSHDOWN=1 cargo run --example issue69_guard_scenario_probe -p ermya-graph
//! ```
//!
...
use ermya_graph::{Graph, Properties, props};
...
fn execute(graph: &Graph, q: &str) -> usize {
    let query = ermya_graph::gql::parse(q).unwrap();
    ermya_graph::gql::execute(graph, &query, 0).unwrap().len()
}
...
const ALL_ARMS: [&str; 7] = [
    "ERMYA_DISABLE_PUSHDOWN",
    "ERMYA_DISABLE_INTERSECTION",
    "ERMYA_DISABLE_LABEL_FASTPATH",
    "ERMYA_DISABLE_ARC_BINDINGS",
    "ERMYA_DISABLE_SORTKEY_PRECOMPUTE",
    "ERMYA_DISABLE_DOUBLE_BUFFER",
    "ERMYA_DISABLE_EDGE_LABEL_FILTER",
];
    ...
    for (label, edge_props) in [
        ...
        repeat(label, || {
            ...
            for _ in 0..200 {
                let edges = g
                    .neighbors(hub)
                    .direction(ermya_graph::Direction::Outgoing)
                    .label("KNOWS")
                    .collect()
                    .unwrap();
